...
task :retrieve_db_config do
  ...
  get(db_config, "/tmp/madek_ethz_db_config.yml")
  dbconf = YAML::load_file("/tmp/madek_ethz_db_config.yml")["production"]
...
task :migrate_database do
  ...
  run "mysqldump --user=#{sql_username} --password=#{sql_password} -r #{backup_dump_path} #{sql_database}"
  ...
  run "mysqldump --user=#{sql_username} --password=#{sql_password} -r #{current_dump_path} #{sql_database}"
