...
export function sourceTransformPlugin(isDev = false) {
  ...
  return {
    ...
    async transform(code, id) {
      ...
      // ── 0. UMD fix: Rolldown's __commonJSMin doesn't inject `require` into
      //    module callbacks, so `typeof require === "function"` is false inside
      //    CJS wrappers (e.g. moment locale files). Remove this condition so the
      //    CJS path is always taken when `exports` and `module` are both defined.
      if (code.includes('typeof require') && code.includes('typeof exports')) {
        const fixed = code.replace(/&&\s*typeof require\s*===\s*["']function["']/g, '')
        if (fixed !== code) {
          code = fixed
          changed = true
        }
