  ...
  class << self
      ...
      ActiveRecord::Base.transaction do
        ...
        InstitutionalGroup.find_in_batches do |db_igroups|
          db_igroups.each do |db_igroup|
            ...
            Rails.logger.warn \
              "Database institutional_groups includes #{db_igroup} \
               but import institutional_groups does not"
          end
        end

        import_igroups.each do |igroup|
          import_or_update! igroup
        end
      end
    end
    ...
    def update_and_log!(igroup)
      ...
      if not db_igroup.persisted?
        ...
        Rails.logger.info \
          "The InstitutionalGroup #{igroup} will be updated, \
           from #{db_igroup_relevant_attributes}"
      end
