namespace :app do
  ...
  namespace :thumbnails do
    ...
    desc 'generate base thumbnails with extensions described in \
          config/mime_icons.yml'
    task :generate do
      ...
      yaml['icons'].each do |type|
        type['extensions'].each do |extension|
          ...
          target_file = File.join(dir,
                                  [File.basename(type['icon'],
                                                 base_extension),
                                   '.',
                                   extension,
                                   base_extension].join)
          ...
          `convert "#{source_file}" \
           -pointsize #{pointsize} \
           -gravity North \
           -font 'lib/fonts/OpenSans-Regular.ttf' \
           -fill '#767676' \
           -annotate +0+#{50 + offset} '#{extension}' "#{target_file}"`

          # SMALL
          source_file = source_file.gsub(/\.png$/, '_small.png')
          target_file = File.join(dir,
                                  [File.basename(type['icon'], base_extension),
                                   '_small',
                                   '.', extension, base_extension].join)
          ...
          -font 'lib/fonts/OpenSans-Regular.ttf' \
          -fill '#767676' \
          -annotate +0+#{21 + offset} '#{extension}' "#{target_file}"`
