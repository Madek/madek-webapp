...
module Resource
  ...
  TS_FIELDS = {:user => lambda {|i| i.user } }
  ...
  TS_ATTRIBUTES = {:sphinx_internal_id => lambda {|i| i.sphinx_internal_id }, :class_crc => lambda {|i| i.class_crc }, :user_id => lambda {|i| i.user_id }, 
                   :updated_at => lambda {|i| i.updated_at.to_i }}
    ...
    def base.to_sphinxpipe(delta = 0)
      ...
      xml.tag!("sphinx:docset") do
        xml.tag!("sphinx:schema") do
            ...
            self::TS_FIELDS.keys.each do |field|
