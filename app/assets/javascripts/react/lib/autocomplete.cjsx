...
initTypeahead = (domNode, resourceType, params, conf, existingValues, existingCallback, onSelect, onAdd, positionRelative)->
  ...
  dataSet = f.merge(searchBackend, {
    ...
    templates: {
      ...
      suggestion: (value) ->
        ...
        if existingValues && f.includes(existingValues(), line) || existingCallback && existingCallback(value)
...
module.exports = React.createClass
  ...
  componentDidMount: ()->
    {resourceType, searchParams, autoFocus, config, existingValues, existingCallback, onSelect, onAddValue, positionRelative} = @props
    conf = f.defaults config,
      minLength: 1
    inputDOM = ReactDOM.findDOMNode(@refs.InputField)
    initTypeahead(inputDOM, resourceType, searchParams, conf, existingValues, existingCallback, onSelect, onAddValue, positionRelative)
