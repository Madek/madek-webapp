...
warning_text = "<p>Ihr Browser wird von dieser Applikation momentan noch nicht unterstützt!</p><br/><p><strong>Es wird empfohlen</strong> für die Benutzung dieser Applikation den <a style='color: grey' href='http://www.mozilla.org/firefox' title='Firefox Browser herunterladen!'>Firefox Browser</a> zu verwenden.</p>"  
...
warning_bar = "<div style='display: block; background: #282723; border-bottom: 1px solid black; color: white; padding: 2px 2px 2px 24px; text-align: center;'><strong>Hinweis:</strong> Ihr Browser wird momentant noch nicht unterstützt. <a style='color: #AAA' href='http://www.mozilla.org/firefox' title='Firefox Browser herunterladen!'>Firefox Browser <img src='https://static-cdn.addons.mozilla.net/media/img/app-icons/med/firefox.png' width='12px' height='12px'/></a> wird empfohlen!</div>"
...
$ ->
  if ! BrowserDetection.is_supported(supported_browser)
    if ! document.cookie.match(/_MAdeK_browser_warning\=1/)
      ...
      warning_style = "border: 3px solid grey; width: "+warning_width+"px; height: "+warning_height+"px; background: white; display: block; position: fixed; top: "+parseInt(($(window).height()/2)-warning_height)+"px; left: "+parseInt(($(window).width()/2)-warning_width/2)+"px; z-index: 10000;"
