class CustomUrlsController < ApplicationController
  ...
  helper_method :url_tranfer_authorized? 
  helper_method :create_url_authorized?

  def url_tranfer_authorized? media_resource, custom_url
  ...
  def create
    begin
      ...
      redirect_to custom_urls_path(@media_resource), flash: {success: "Die URL wurde angelegt."}
      ...
      redirect_to new_custom_url_path(@media_resource,url: params[:url]), 
        flash: {http_code: 422, 
                error:  "Es kann maximal eine URL im Zeitraum von 3 Minuten für eine Resource erzeugt werden. Bitte warten Sie!"} 
  ...
  def transfer_url 
    begin
      @custom_url= CustomUrl.find params[:url]
      @media_resource= MediaResource.find params[:id]
      raise NotAuthorized unless url_tranfer_authorized?(@media_resource,@custom_url)       
      @custom_url.update_attributes! media_resource: @media_resource, is_primary: false, \
        updator: current_user
      redirect_to custom_urls_path(@media_resource), flash: {success: "The URL has been transfered."}
    rescue NotAuthorized => e
      redirect_to confirm_url_transfer_media_resource_path(@media_resource,url: params[:url]), 
        flash: {http_code: 403, error: "Sie sind nicht berechtigt diese URL zu transferieren!"}
    end
  end
  ...
  def set_primary_url
    begin
      ...
      redirect_to custom_urls_path(@media_resource), flash: {success: "Eine neue primäre URL wurde gesetetzt!"}
    rescue NotAuthorized => e
