class MyController < ApplicationController
  ...
  def dashboard
    ...
    @latest_media_entries = \
      current_user.media_entries.reorder('updated_at DESC').limit(limit)
    @latest_collections = \
      current_user.collections.reorder('updated_at DESC').limit(limit)
    @latest_filter_sets = \
      current_user.filter_sets.reorder('updated_at DESC').limit(limit)
    ...
    @entrusted_media_entries = \
      MediaEntry.entrusted_to_user(current_user)
        .reorder('updated_at DESC').limit(limit)
