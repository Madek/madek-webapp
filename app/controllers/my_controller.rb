class MyController < ApplicationController
  ...
  before_filter do
    ...
    @user_entrusted_resources_count = MediaResource.filter(current_user).entrusted_to_user(current_user,:view).count
  ...
  def dashboard
    ...
    @latest_user_imports = MediaResource.filter(current_user, {:meta_data => {:"uploaded by" => {:ids => [current_user.id]}}}).ordered_by(:created_at).limit(11)
    @user_entrusted_resources = MediaResource.filter(current_user).entrusted_to_user(current_user,:view).ordered_by(:updated_at).limit(11)
