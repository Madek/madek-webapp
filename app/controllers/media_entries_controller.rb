class MediaEntriesController < ApplicationController
  ...
  def select_collection
    ...
    modal_presenter = Presenters::MediaEntries::MediaEntrySelectCollection.new(
      current_user, media_entry, search_term)

    @get = Presenters::MediaEntries::MediaEntryShow.new(
      media_entry,
      current_user,
      user_scopes_for_media_resource(media_entry),
      list_conf: resource_list_params, modal_presenter: modal_presenter)

    respond_with(@get, template: 'media_entries/show')
  end

  def add_to_collection
    media_entry = get_authorized_resource

    collection_selection = read_checkboxes(params.require(:selected_collections))
  ...
  def read_checkboxes(checkboxes)
    collections = {}
    checkboxes.each do |key, checks|
      collections[key] = checks.include?('true')
    end
    collections
  end
