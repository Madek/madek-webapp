...
#    @json = Logic.data_for_page(@media_entries, current_user).to_json
    ...
    scope, viewable_ids = if @user
      ...
      intersected_ids = all_ids & viewable_ids

      @json = Logic.data_for_page2(intersected_ids.paginate(:page => params[:page]), current_user).to_json

    ########################################################################

    #temp# @facets = MediaEntry.facets params[:query], :match_mode => :extended2, :with => with
