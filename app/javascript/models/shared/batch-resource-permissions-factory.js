...
module.exports = function(name, baseModel){ return baseModel.extend({
  type: name,
  ...
  initialize(props){
    ...
    PERMISSIONS.forEach((...args)=> {
      ...
      const batchPerms = f.map(permsBySubject, function(perms, uuid){
        const combinedPerms = f.object(f.map(permissionTypes, function(key){
      ...
      return this[permissionName].set(
        modelType === 'model' ? batchPerms[0] : batchPerms);
    });
  ...
  serialize() {
    ...
    return {
      ...
      permissions: f.object(f.map(permissionSubjects, function(key) {
        let list = key === 'public_permission' ? [data[key]] : data[key];
        list = f.compact(f.map(list, function(permissions) {
          // cleanup mixed:
          const perms = f.mapValues(permissions, function(permission){
            if (permission === 'mixed') { return undefined; } else { return permission; }
          });
          // cleanup subjects:
