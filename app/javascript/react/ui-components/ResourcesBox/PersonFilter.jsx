...
function getStaticSource(staticItems) {
  return (term, syncCallback) => {
    const isMatch =
      term.length === 0 ? () => true : u => u.label.toLowerCase().includes(term.toLowerCase())
    syncCallback(f.filter(staticItems, user => !user.selected && isMatch(user)))
  }
}
...
function getRemoteSource(staticItems, contextKeyId, jsonPath) {
  ...
  return (query, syncCallback, asyncCallback) => {
    if (!_source) {
      _source = createRemoteSource(getUrl(), {
    ...
    if (!query) {
      ...
      syncCallback(staticItems.filter(x => !x.selected).slice(0, maxHits))
    } else {
      _source(query, syncCallback, asyncCallback)
    }
  }
}

// Use React rendering — no need to escape, JSX handles it
