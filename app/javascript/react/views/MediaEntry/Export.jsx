...
class MediaEntryExport extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      checksumGeneratedAt: props.get.media_file.checksum_generated_at,
  ...
  handleGenerateChecksum = async () => {
    ...
    try {
      ...
      this.setState({
        ...
        checksumGeneratedAt: data.checksum_generated_at,
        isGenerating: false,
        checksumMatch: null
      })
    } catch (err) {
  ...
  render() {
    ...
    const sectionChecksum = (
      <div className="ui-export-block">
        ...
        {!checksum && <div className="col1of5">{t('media_entry_export_checksum_empty')}</div>}
        {!checksum && <div className="col3of5 by-center">sha256</div>}
        ...
        {checksum && (
          <div className="col4of5">
            ...
            {checksumVerifiedAt && (
              <div>
                {t('media_entry_export_checksum_verified_at')}{' '}
                {Moment(checksumVerifiedAt).format('L')}
              </div>
            )}
            {!checksumVerifiedAt && checksumGeneratedAt && (
              <div>
                {t('media_entry_export_checksum_generated_at')}{' '}
                {Moment(checksumGeneratedAt).format('L')}
              </div>
            )}
          </div>
        )}
        <div className="col1of5 by-right">
          {checksum && (
            <button
