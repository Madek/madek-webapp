...
class BoxMetaKeySelector extends React.Component {
  ...
  findMetaKeysWithTypes(metaKeyIds) {
    ...
    return l.map(metaKeyIds, k => l.find(metaKeysWithTypes, mkt => mkt.metaKeyId == k))
  }
  ...
  renderTemplateEntries() {
    return l.map(this.prepareContexts(), pc => {
      ...
      var renderBubbles = () => {
        ...
        var keys = l.map(contextKeys, ck => {
          return {
            metaKey: ck.meta_key,
            contextKey: ck
          }
        })
        return <BoxBatchEditFormKeyBubbles keys={keys} onClickKey={this.props.onClickKey} />
      }
        ...
        <div key={context.uuid}>
          <div>
            <div style={{ marginBottom: isSelected() ? '10px' : '0px' }}>
              ...
              <div style={{ marginLeft: '20px', marginTop: '5px' }}>{renderBubbles()}</div>
            </div>
          </div>
        </div>
      )
  ...
  renderVocabularies() {
    ...
    var vocabularies = l.reduce(
      ...
      (memo, mmd) => {
        l.each(mmd.data.vocabularies_by_vocabulary_id, (v, k) => (memo[k] = v))
        return memo
      },
      {}
    )
    ...
    var vocabMetaKeys = l.reduce(
      ...
      (memo, vocab2Key) => {
        l.each(vocab2Key, (v, k) => (memo[k] = l.uniq(l.concat(memo[k] ? memo[k] : [], v))))
        return memo
      },
      {}
    )
    ...
    return l.map(vocabularies, (v, k) => {
      ...
      var renderBubbles = () => {
        ...
        var keys = l.map(this.findMetaKeysWithTypes(vocabMetaKeys[k]), mkt => {
          return {
            metaKey: mkt.metaKey,
            contextKey: null
          }
  ...
  prepareContexts() {
    ...
    var contextsWithType = () => {
      ...
      return l.map(metaMetaData.meta_data_edit_context_ids, cid => {
        return {
          type: type(),
    ...
    return l.filter(
      l.uniqBy(contextsWithType(), ct => ct.context.uuid),
    ...
    return (
      <div>
        ...
        <div className="ui-container tab-content bordered bright rounded-right rounded-bottom">
          <div className="phs pts">{renderTabContent()}</div>
        </div>
      </div>
    )
  }
  ...
  render() {
    return <div>{this.renderContextTabs()}</div>
  }
}
