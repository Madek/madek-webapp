...
const ActionsDropdown = ({ parameters, callbacks }) => {
  const showActions = ActionsDropdownHelper.showActionsConfig(parameters)

  if (!Object.values(showActions).some(Boolean)) {
    return null
  }
  ...
  const createHoverActionItem = (enableEntryByOnClick, hoverId, count, icon, text) => (
    <Dropdown.MenuItem
      ...
      onMouseEnter={() => callbacks.onHoverMenu(hoverId)}
      onMouseLeave={() => callbacks.onHoverMenu(null)}>
      <Icon
        i={icon}
        mods="ui-drop-icon"
        style={{
      ...
      <Dropdown.Menu className="ui-drop-menu">
        ...
        {showActions.addToSet &&
          createHoverActionItem(
            nofSelected > 0 ? () => callbacks.onBatchAddToSet(selection) : undefined,
            'add_to_set',
            nofSelected,
            'move',
        ...
        {showActions.removeFromSet &&
          createHoverActionItem(
            nofSelected > 0 ? () => callbacks.onBatchRemoveFromSet(selection) : undefined,
            'remove_from_set',
            nofSelected,
            'close',
        ...
        {(() => {
          if (showActions.edit) {
            if (
              ...
              return createHoverActionItem(
                present(batchEditables) ? () => callbacks.onBatchEdit(batchEditables) : undefined,
                'media_entries_edit',
          ...
          if (showActions.edit) {
            ...
            return createHoverActionItem(
              batchEditables && batchEditables.length > 0 && batchEditables.length <= 12
                ? () => callbacks.onBatchEditTitle(batchEditables.map(entry => entry.uuid))
                : undefined,
        ...
        {(() => {
          if (showActions.editSets) {
            if (
              ...
              if (selection) {
                batchSetEditables = SelectionScope.batchMetaDataResources(selection, ['Collection'])
              }
              return createHoverActionItem(
                present(batchSetEditables)
                  ? () => callbacks.onBatchEditSets(batchSetEditables)
                  : undefined,
                'collections_edit',
          ...
          if (showActions.deleteResources) {
            ...
            return createHoverActionItem(
              present(batchDestroyables)
                ? () => callbacks.onBatchDeleteResources(batchDestroyables)
                : undefined,
              'resources_destroy',
          ...
          if (showActions.managePermissions) {
            ...
            return createHoverActionItem(
              present(batchPermissionEditables)
                ? () => callbacks.onBatchPermissionsEdit(batchPermissionEditables)
                : undefined,
          ...
          if (showActions.managePermissionsSets) {
            ...
            return createHoverActionItem(
              present(batchPermissionSetsEditables)
                ? () => callbacks.onBatchPermissionsSetsEdit(batchPermissionSetsEditables)
                : undefined,
          ...
          if (showActions.transferResponsibility) {
            ...
            return createHoverActionItem(
              present(batchTransferResponsibilityEditables)
                ? () =>
                    callbacks.onBatchTransferResponsibilityEdit(
                      batchTransferResponsibilityEditables
                    )
                : undefined,
          ...
          if (showActions.transferResponsibilitySets) {
            ...
            return createHoverActionItem(
              present(batchTransferResponsibilitySetsEditables)
                ? () =>
                    callbacks.onBatchTransferResponsibilitySetsEdit(
                      batchTransferResponsibilitySetsEditables
                    )
                : undefined,
        ...
        {showActions.addToClipboard
          ? nofSelected === 0
            ...
            : createHoverActionItem(
                nofSelected > 0
                  ? () => callbacks.onBatchAddSelectedToClipboard(selection)
                  : undefined,
        ...
        {showActions.removeFromClipboard
          ? nofSelected === 0
            ...
            : createHoverActionItem(
                nofSelected > 0 ? () => callbacks.onBatchRemoveFromClipboard(selection) : undefined,
