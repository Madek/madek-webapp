...
module.exports = merged => {
  ...
  var nextNewWidget = () => {
    var supportedTypes = l.filter(['Person', 'PeopleGroup'], t =>
      l.includes(nextProps.metaKey.allowed_people_subtypes, t)
    )
  ...
  var nextKeywords = () => {
    var keywordMatch = (k, r) => {
      if (k.id && r.id && k.id == r.id) {
        ...
        return (
          k.first_name == r.first_name && k.last_name == r.last_name && k.pseudonym == r.pseudonym
        )
    ...
    if (event.action == 'remove-keyword-by-id') {
      ...
      var r = event.keyword
      return l.reject(data.keywords, k => keywordMatch(k, r))
