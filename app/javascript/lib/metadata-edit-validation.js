...
module.exports = {
  ...
  _validityForMetaKeyIds(meta_meta_data, models, meta_key_ids) {
    const mandatory_meta_key_ids = f.keys(meta_meta_data.mandatory_by_meta_key_id);
    const reduced_mandatories = f.filter(meta_key_ids, meta_key_id => f.include(mandatory_meta_key_ids, meta_key_id));
  ...
  _changesPerContext(meta_meta_data, models, context_id) {
    ...
    f.each(models, (model, meta_key_id) => {
      if ((context_id && (f.includes(this._meta_key_ids_by_context_id(meta_meta_data, context_id), meta_key_id))) || !context_id) {
