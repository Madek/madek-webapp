        ...
        included do
          ...
          def _get_video_previews(previews)
            ...
            ['webm', 'mp4'].map do |format|
              video = previews.where(content_type: "video/#{format}")
                .reorder(width: :desc, created_at: :desc).first
              Presenters::Previews::Preview.new(video) if video.present?
            end.compact
          end

          def _get_image_preview(previews, size)
            # Get the just wanted width from the legacy interal "size classes"!
            wanted_size = Madek::Constants::THUMBNAILS[size]
            raise 'invalid size!' unless wanted_size.present?
            wanted_width = wanted_size[:width]

            if previews.present? and previews[0].media_file.representable_as_image?
              # select newest, smallest previews that are AT LEAST the wanted size:
              previews = previews
                .where('previews.width >= ?', wanted_width)
                .reorder(width: :asc, created_at: :desc)
              # BUT if that doesnt exist, get the LARGEST there are
              previews ||= previews
                .reorder(width: :desc, created_at: :desc)
              # select first or apply 30% rule for videos
              image = _get_first_or_30_percent(previews) if previews.present?
            end
          ...
          def _get_first_or_30_percent(previews)
            ...
            if (previews.length > 1 and is_from_video)
              ...
              frames = previews.group_by(&:width).first.second.sort_by(&:filename)
