...
describe MyController do
  ...
  before :example do
    ...
    4.times { FactoryGirl.create :media_entry_user_permission, arg_hash.merge(user: @user, media_entry: FactoryGirl.create(:media_entry)) }
    4.times { FactoryGirl.create :media_entry_group_permission, arg_hash.merge(group: group, media_entry: FactoryGirl.create(:media_entry)) }

    4.times { FactoryGirl.create :collection_user_permission, arg_hash.merge(user: @user, collection: FactoryGirl.create(:collection)) }
    4.times { FactoryGirl.create :collection_group_permission, arg_hash.merge(group: group, collection: FactoryGirl.create(:collection)) }

    4.times { FactoryGirl.create :filter_set_user_permission, arg_hash.merge(user: @user, filter_set: FactoryGirl.create(:filter_set)) }
    4.times { FactoryGirl.create :filter_set_group_permission, arg_hash.merge(group: group, filter_set: FactoryGirl.create(:filter_set)) }
  ...
  it 'dashboard' do
    ...
    expect(entrusted_media_entries.first).to eq MediaEntry.entrusted_to_user(@user).reorder('updated_at DESC').first

    entrusted_collections = assigns(:entrusted_collections)
    expect(entrusted_collections.count).to be == @limit
    expect(entrusted_collections.first).to eq Collection.entrusted_to_user(@user).reorder('updated_at DESC').first
    ...
    expect(entrusted_filter_sets.first).to eq FilterSet.entrusted_to_user(@user).reorder('updated_at DESC').first
  end
