...
describe "/media_sets", :type => :api do
  ...
  context "im NOT logged in or authenticated" do
    ...
    it "should return the public splashscreen media_set containing its media_entries with author, image and title", do
      ...
      json["id"].should be media_set.id
      json["media_entries"].size.should be media_set.media_entries.size
      
      json["media_entries"].each do |media_entry|
        media_entry["id"].blank?.should_not be true
        media_entry["image"].blank?.should_not be true
        media_entry["author"].blank?.should_not be true
        media_entry["title"].blank?.should_not be true
      end
    end
  end
  ...
  context "im logged in or authenticated" do
    ...
    it "should return media_resources of the child_set with images", do
      ...
      json["id"].should be parent_media_set.id
      json["media_resources"].size.should be parent_media_set.media_entries.size+parent_media_set.child_sets.size
      
      json["media_resources"].each do |entry|
        entry["id"].blank?.should_not be true
        entry["image"].blank?.should_not be true
        entry["type"].blank?.should_not be true
      end
    end
  end
end