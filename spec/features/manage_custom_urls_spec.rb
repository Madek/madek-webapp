...
feature 'manage custom urls' do
  ...
  scenario 'check cross transfer' do
    prepare_user
    login
    media_entry = create_media_entry('Media Entry')
    collection = create_collection('Collection')
    scenario_check_cross_transfer(media_entry, collection)
  end
  ...
  def scenario_table_content(resource)
    ...
    click_single_set_as_primary_button('address3')
  ...
  def scenario_check_cross_transfer(resource1, resource2)
  ...
  def scenario_set_primary(resource)
    ...
    click_submit_button
    check_overview_custom_url(resource, 'first_address_name')
    click_single_set_as_primary_button('second_address_name')
  ...
  def click_single_set_as_primary_button(address_name)
  ...
  def check_empty_addresses(resource)
    find(
      '.ui-body-title',
      text: concat_k1_v1(:custom_urls_title, resource.title))

    expect(page).to have_content(I18n.t('custom_urls_no_addresses_defined'))
  end
