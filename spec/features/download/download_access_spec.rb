...
feature 'Download - Access', browser: :headless do
  context 'Owner' do
    before :each do
      ...
      begin 
        @media_entry = FactoryGirl.create(
          :media_entry_with_large_image_media_file, user: @current_user
        )
    ...
    scenario 'Download should be accessible for owner' do
      ...
      within('#with-latest-meta-data') do
        @download_path = find('.ui-drop-item:nth-child(2) a', visible: false)[:href]
      end
      expect(@download_path).to include 'size=x_large'
      visit @download_path
    ...
    scenario 'Download of the original file should be accessible for owner' do
      ...
      click_on_text 'Datei mit aktuellen Metadaten'
      within('#with-latest-meta-data') do
        @download_path = find('.ui-drop-item:first-child a', visible: false)[:href]
      end
      expect(@download_path).not_to include 'size=x_large'
      visit @download_path
      expect(page.response_headers['Content-Type']).to eq 'image/jpeg'
    end
  end
  ...
  context 'User' do
    before :each do
      ...
      begin
        @media_entry = FactoryGirl.create(
          :media_entry_with_large_image_media_file, user: User.last
        )
      end until @media_entry.media_file.previews.where(thumbnail: 'x_large').present?
    end
    ...
    scenario 'Download of the original file should be inaccessible for entries without permissions' do
      visit "/download?id=#{@media_entry.id}&update=1"
