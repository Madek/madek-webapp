...
$.widget("ui.multiselect", {
  options: {
	...
	_create: function() {
		...
		this.availableActions.find('input').width(Math.max(this.availableActions.width() - this.availableActions.find('a.add-all').width() - 30, 110)); //sellittf// 20
			...
			if (data = this.options.dataParser.call(this, data)) {
				for (var key in data) {
					...
					if (this.element.find('option[value="'+key+'"]').size()==0) {
						...
						elements.push( $('<option value="'+key+'" selected="selected"/>').text(data[key].value).appendTo(this.element)[0] );
						//sellittf end//
					}
				}
			}
    ...
    **************************************/
	...
	_insertToList: function(node, list) {
		...
		var _addNode = function() {
			...
			try {
				if (false) { //sellittf//avoid sorting// (succ) {
